<template>
	<div class="maincon">
		...
		<!-- 蒙尘 -->
		<el-dialog title="新增渠道" append-to-body :visible.sync="dialogVisible" width="30%">
			<div class="chen">
...
<script>
	...
	import Table from '@/components/details/Channletable.vue'
	import {excalcreat,excalquery} from '@/utils/login.js'
	import {channelset,channelquery} from '@/utils/mchinfo.js'
	export default {
		inject: ["reload"],
		components: {
			Table,
		...
		methods: {
			...
			submitForm(formName){
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						channelquery(this.ruleForm.channelName).then(data=>{
							if(data.data.data.length == 0){
								this.ruleForm.createAt=this.conversion_time1
								channelset(this.ruleForm).then(res=>{
									this.$message.success('信息储存成功')
									this.dialogVisible=false
									this.reload()
								})
							}else{
								this.$message.error('渠道已存在')
							}
						})
					} else {
