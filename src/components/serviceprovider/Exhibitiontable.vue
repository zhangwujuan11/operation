		...
		<el-table :data="tableData" style="width:100%;">
			...
			<el-table-column label="操作">
				<template slot-scope='scope'>
					<span class="control" @click=chekfun(scope.row)>确认</span>
					<span class="control" style="color: red;" @click=chekwrong(scope.row)>重算</span>
				</template>
			</el-table-column>
		</el-table>
		<!-- 蒙尘 -->
...
<script>
	...
	import {serviceprovilist,provideright} from '@/utils/serviceprovider.js'
	export default {
		inject: ["reload"],
		...
		created() {
			serviceprovilist().then(res=>{
				this.tableData=res.data.data.unverified
			...
			chekfun(n){
				provideright({
					...
					status:'right'
					 ...
					 this.$message.error("确认失败，请联系相关人员")
				})
			},
			// 重算
			chekwrong(n){
				provideright({
					...
					status:'wrong'
					 ...
					 this.$message.success("已提交重算")
					  this.reload()
				}).catch(()=>{
					 this.$message.error("重算失败请联系相关人员")
				})
			}
		}
	}
